...
POSTGRES_PASSWORD="$(kubectl get secret -n "$NAMESPACE" "${RELEASE}-postgresql" -o jsonpath='{.data.password}' | base64 -d)"
...
kubectl exec -n "$NAMESPACE" "$POD_NAME" -- env PGPASSWORD="$POSTGRES_PASSWORD" psql -U "$DB_USER" -d "$DB_NAME" -f /tmp/00-schema.sql
kubectl exec -n "$NAMESPACE" "$POD_NAME" -- env PGPASSWORD="$POSTGRES_PASSWORD" psql -U "$DB_USER" -d "$DB_NAME" -f /tmp/10-seed-dev.sql
